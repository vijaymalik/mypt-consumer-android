package co.com.mypt.PlanRenewal.Upgrade
...
class ValidityOfPackageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_validity_of_package)
        ...
        minDays = intent.getIntExtra("totalDays",0)
        progressDays = intent.getIntExtra("totalDays",0)
        sessions = intent.getStringExtra("sessions").toString()
        ...
        tvcontinue.setOnClickListener{
            val intent1 = Intent(this@ValidityOfPackageActivity, UpgradeTopUpReviewPackageActivity::class.java)
            intent1.putExtra("typeSubsctiption",intent.getStringExtra("typeSubsctiption"))
            intent1.putExtra("id",intent.getStringExtra("id"))
            intent1.putExtra("sessions",sessions)
            intent1.putExtra("days","$progressDays")
            intent1.putExtra("price",price)
            startActivity(intent1)
        }
        cardEdit.setOnClickListener{
            /*val intent = Intent(this@ValidityOfPackageActivity, TrainersListActivity::class.java)
        ...
        seekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            ...
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                if(progress <= minDays){
                    seekBar!!.progress = minDays
                    setSeekbarData(minDays,seekBar)
                ...
                setSeekbarData(progress,seekBar)
    ...
    private fun setSeekbarData(progress: Int, seekBar: SeekBar?) {
        val startColor = Color.parseColor("#3DD772")
        val middleColor = Color.parseColor("#42AB98")
        val endColor = Color.parseColor("#4981F2")
        if(progress > 1)
            sessionValue.text = "$progress days"
        else
            sessionValue.text = "$progress day"

        progressDays = progress - minDays
     ...
     private fun getSessionData(checkedType: String) {
       val progressDialog: Dialog = ProgressDialog.progressDialog(this@ValidityOfPackageActivity,"")
       progressDialog.show()

         Log.e("progressDays","$progressDays")
         GetMethod(ApiURL.upgradeTopUpPlan+"upgrade&sessions=$sessions&id=${intent.getStringExtra("id")}&days=$progressDays",
             this@ValidityOfPackageActivity).startMethod(object :
             ResponseData {
             override fun response(data: String?) {
                 progressDialog.dismiss()

                 Log.e("upgradeValidityResp",data.toString())
                 try {
