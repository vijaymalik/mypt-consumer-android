...
class CommonPaymentSelectionActivity : AppCompatActivity() {
    var payment_id=""
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_common_payment_selection)
        ...
        tvTotalPrice.text = "Total: AED ${intent.getStringExtra("price")}"

        tvPaymentPrice.text = "Pay AED ${intent.getStringExtra("price")}"
        back_1.setOnClickListener{
            finish()
        }
        im_Info.setOnClickListener{
            var intent=Intent(this,TabbyWebViewActivity::class.java)
            val amount = getIntent().getStringExtra("price")?.replace(" ", "")
            Log.e("amount",""+amount)
        ...
        linearPay.setOnClickListener{
            if (isSelected){
                if (selectedPaymentOption == "debit"){
                    val intent = Intent(this, PlanRenewalCCAvenueWebPage::class.java)
    ...
    private fun billBottomSheet() {
        ...
        tvTax.text = "AED "+intent.getStringExtra("tax_price")
    ...
    private fun startTabbyCheckout() {
        ...
        val tabbyPayment = TabbyPayment(
            amount = BigDecimal(intent.getStringExtra("price").toString().toDouble()),
    ...
    private fun onCheckoutResult(tabbyResult: TabbyResult) {
        ...
        when (tabbyResult.result) {
            TabbyResult.Result.AUTHORIZED -> {
                ...
                val intent = Intent(this@CommonPaymentSelectionActivity,
                    CommonBookingConfirmActivity::class.java)
                intent.putExtra("id",getIntent().getStringExtra("id").toString())
                intent.putExtra("sessions",getIntent().getStringExtra("sessions").toString())
                intent.putExtra("days",getIntent().getStringExtra("days").toString())
                intent.putExtra("price",getIntent().getStringExtra("price").toString())
    ...
    private fun alerDialog(message:String){
        val builder = AlertDialog.Builder(this@CommonPaymentSelectionActivity)
