...
class SemiCircleProgressViewHealth @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null
) : View(context, attrs) {
    private var maxValue = 0f
    private val strokeWidth = 20f  // Large thickness for progress bar
    private val cornerRadius = 5f // Rounded effect at the ends
    private val backgroundPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = context.getColor(R.color.progress_default)
        style = Paint.Style.STROKE
        strokeWidth = this@SemiCircleProgressViewHealth.strokeWidth
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        shader?.let {
            ...
            canvas.drawArc(rectF, 180f, (progress / maxValue) * 180, false, progressPaint)
        }
    }
