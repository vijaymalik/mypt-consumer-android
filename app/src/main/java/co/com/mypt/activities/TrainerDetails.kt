...
class TrainerDetails : AppCompatActivity(), ViewTreeObserver.OnScrollChangedListener {
...
//        bookSlot = findViewById(R.id.bookSlot)
        ...
        try {
            Log.e("studiodetail",""+intent.getStringExtra("studio_id"))
        }catch (e: Exception){
            e.printStackTrace()
        }
                ...
                try {
                    ...
                    if (jsonObj.optBoolean("status")){
                       val jsonData=jsonObj.optJSONObject("data")
                        Glide.with(this@TrainerDetails).load(jsonData!!.optString("profile")).fitCenter().into(trainerImage)
...
//                            follow_1.text = resources.getString(R.string.unfollow)
                        ...
                        specialitiesRecyclerView.adapter = SpecialitiesAdapter(this@TrainerDetails,specialitiesArrayList)
                        ...
                        if (jsonData.optJSONArray("galleries").length()>0){
                            ...
                            galleryRecyclerView.adapter = GalleryAdapter(this@TrainerDetails,galleryArrayList)
                        }
                        else{
                        ...
                        /*bookSlot.setOnClickListener {
                            if (intent.getStringExtra("haveSlot")!="no"){
                                if (sharedPreferences.getString("typeWorkout","").equals("home")){
                                    ...
                                    val intent1 = Intent(this@TrainerDetails, BookSlot::class.java)
                                    intent1.putExtra("type",intent.getStringExtra("type"))
                                    intent1.putExtra("trainer_id",intent.getStringExtra("trainer_id"))
                                    intent1.putExtra("studio_id",intent.getStringExtra("studio_id"))
